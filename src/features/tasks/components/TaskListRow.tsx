...
export const TaskListRow = ({ task, density = 'COMFORTABLE', onViewDetails, onOpenProject }: { task: TaskEntity; density?: DensityMode; onViewDetails: () => void; onOpenProject?: () => void }) => {
  ...
  return (
    <div
      ...
      className={cn(
        "group flex items-center border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-all duration-200 px-4 relative overflow-hidden cursor-default",
        isCompact ? "py-2.5" : "py-4",
        isRightClickAnimated && 'ring-2 ring-indigo-200 bg-indigo-50/40 scale-[1.005]'
      )}
    > 
